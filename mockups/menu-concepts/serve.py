...
# six editors saving what they were opened to edit — the six camera routes, the walkers' loops,
# the firefly knots, the boats' laps, the steam vents and the wind in the trees. All are OVERLAYS
# over the HTML's own tables, not rewrites of them: the page lays each over its own table at boot
# and deleting the file is a full undo, so nothing here can damage anything that is not
# reproducible from the HTML alone.
SAVES = {'__flights': 'flights.json', '__loops': 'loops.json', '__flies': 'fireflies.json',
         '__boats': 'boats.json',
         '__steam': 'steam.json', '__sway': 'sway.json'}
SAVE_MAX = 64 * 1024
