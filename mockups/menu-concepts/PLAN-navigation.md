...
Plus **Look it up** 引く — an overlay on `/` from any level, not a section. **Built 2026-08-31.** One field; the route is inferred from what you type and the five run along the foot so the inference is never a guess. Two of them — the tutor and the OCR workbench — need a model running and are drawn as doors the mockup does not walk through. **Settings is the second overlay, built the same day** — on `,`, sharing this one's shell and mutually exclusive with it. It lists `AppSettingsModal`'s fifteen real sections and draws no controls: that modal is finished, and what the menu needs from it is placement plus the one fact that two of the fifteen are optional downloads — which is why the lookup answers a kanji and not a word.
