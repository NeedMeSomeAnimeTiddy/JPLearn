...
from domain.scheduler import AGAIN, GOOD, ReviewState, update
...
class HomeFrame(AppFrame):
    def __init__(self, master: "App") -> None:
        ...
        layout.addWidget(
            _button(
                "ひ  Hiragana - Flashcards",
                "#6c3483",
                lambda: self.app.start_review("hiragana", mode="flashcard"),
            )
        )
        layout.addWidget(
            _button(
                "ひ  Hiragana - Multiple Choice",
                "#8e44ad",
                lambda: self.app.start_review("hiragana", mode="multiple_choice"),
            )
        )
        layout.addWidget(
            _button(
                "カ  Katakana - Flashcards",
                "#1a5276",
                lambda: self.app.start_review("katakana", mode="flashcard"),
            )
        )
        layout.addWidget(
            _button(
                "カ  Katakana - Multiple Choice",
                "#2471a3",
                lambda: self.app.start_review("katakana", mode="multiple_choice"),
            )
        )
...
class FlashcardFrame(AppFrame):
    ...
    def _rate(self, quality: int) -> None:
        ...
        if quality == AGAIN:
            ...
            self._correct += 1
        state = update(self._states[current.id], quality)
        database.save_state(deck.name, state)
        database.log_review(deck.name, current.id, quality)
        self._next_card()
...
class StatsFrame(AppFrame):
    ...
    def _render_cards(self) -> None:
        ...
        for key, loader in ALL_DECKS.items():
            ...
            deck_title.setStyleSheet(f"color: {deck_colours.get(key, HIGHLIGHT)};")
...
class App(QMainWindow):
    ...
    def start_review(self, deck_key: str, mode: str = "flashcard") -> None:
        ...
        database.init_db()
        due = [card for card in deck.cards if database.load_states(deck.name, [card.id])[card.id].is_due()]
        if not due:
